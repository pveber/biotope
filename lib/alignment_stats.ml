...
let summary ~sample_name ~mapped_reads samples =
  Workflow.path_plugin ~descr:"alignment_stats.summary" (

    let%pdeps stat_files = path_list @@ List.map samples ~f:(fun s -> bamstats (mapped_reads s))
    and          samples = data @@ List.map samples ~f:sample_name
    and    chrstat_files = path_list @@ List.map samples ~f:(fun s -> chrstats (mapped_reads s))
    in
